...
export class Soql {
  ...
  public query(qry: string): Promise<QueryResult<any>> {
  ...
  public queryMore(locator: string): Promise<QueryResult<any>> {
    return this.client.request({
      method: 'GET',
      url: locator,
      timeout: 30 * 1000,
  ...
  public async bulkQuery(qry: string): Promise<IBulkQueryResult> {
  ...
  private wait(millisec: number) {
    return new Promise((resolve) => setTimeout(resolve, millisec));
  }
}

export interface IBulkQueryResult {
  id: string;
  operation: string;
  object: string;
  createdDate: string;
