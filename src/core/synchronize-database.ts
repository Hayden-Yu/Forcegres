...
import { IBulkQueryResult } from '../lib/salesforce/soql';
...
export function initalizeDatabase() {
  const schema = readFileSync(resolve(__dirname, '../../resources/schema.psql'), 'utf-8')
...
export async function loadScratch(name: string) {
  ...
  return db.transact(async conn => {
    ...
    let sfQuery: IBulkQueryResult | undefined;
    try {
    ...
    if (sfQuery) {
      ...
      await Promise.all([
        loadChunkData(schema, init.records, conn.query.bind(conn)),
        loadDataFollowUp(schema, init.nextRecordsUrl, conn.query.bind(conn)),
      ]);
...
function loadUpdatesByChunk(
  ...
  acc.push(sf.soql.query(query).then(res => {
    return Promise.all([
      loadChunkData(schema, res.records, conn.query.bind(conn)),
      loadDataFollowUp(schema, res.nextRecordsUrl, conn.query.bind(conn)),
    ]) as Promise<any>;
  }));
  ...
  return query(translateQry.loadSyncHistory(name))
    .then(res => {
      ...
      return res.rows.length ?
        (moment.tz(lastUpdate ?
          lastUpdate.enddate
          : res.rows[res.rows.length - 1].enddate, 'UTC').toISOString())
        : '';
    });
}
