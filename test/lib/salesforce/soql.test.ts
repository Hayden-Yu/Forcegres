...
describe('Soql', () => {
  ...
  beforeEach(() => {
    ...
    requestMethod = sandbox.stub(ApiClient.prototype, 'request').callsFake((arg) => {
      ...
      if (arg && (arg as any).url &&
          ((arg as any).url.endsWith(errorQuery) || (arg as any).url.endsWith(errorLocator))) {
  ...
  it('should send query', async () => {
    ...
    chai.assert((requestMethod.getCall(0).args[0] as any).url ===
    '/services/data/' + apiClient.version + '/query?q=' + query,
    'query should send in url');
